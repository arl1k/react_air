...
class App extends Component {
  ...
  ShowReviews(event) {
    if(this.state.reviews.length > 0) { //hide loaded reviews
      this.setState({ reviews: [], reviewButtonDisabled : false }) 
    }
    else {
      this.setState({ reviews: [], reviewButtonDisabled : true }) 
      this.LoadAllReviews(event.currentTarget.getAttribute('data-propid'))
    }
  }
