...
exports.LoadProperties = function(cityName, offset = 0) {
    ...
    return fetch(proxyUrl + searchUrl + "&" + "location=" + encodeURIComponent(cityName) + "&_limit=50" + offsetStr, options)
        .then(response => {
...
exports.LoadReviews = function(propertyId, offset = 0) {
    ...
    return fetch(proxyUrl + reviewUrl +"&listing_id=" + propertyId + "&role=all" + "&_limit=10" + offsetStr, options)
    .then(response => {
        return response.json();
    })
    .then(json => {
